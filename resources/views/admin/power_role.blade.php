...
<body style=" background-color: gainsboro;">
    ...
    <div style="margin:0px; background-color: white; margin:0 10px;">
        <blockquote class="layui-elem-quote">
            <button type="button" class="layui-btn layui-btn-small" id="getAll"><i class="fa fa-plus" aria-hidden="true"></i> 添加角色</button>
            <form class="layui-form" style="float:right;">
                <div class="layui-form-item" style="margin:0;">
                    <label class="layui-form-label">名称</label>
                    <div class="layui-input-inline">
                        <input type="text" name="name" placeholder="角色" autocomplete="off" class="layui-input">
                    </div>
    ...
    <script>
        layui.config({
            ...
            btable.set({
                openWait: true,//开启等待框
                elem: '#content',
                url: "{{asset('admin/api/admin_role')}}", //数据源地址
                pageSize: 10,//页大小
                columns: [{ //配置数据列
                    fieldName: '角色名', //显示名称
                    field: 'name', //字段名
                                    ...
                                    case 'del': //删除
										if(id===1){
                                            ...
                                            layerTips.confirm('确定要删除[ <span style="color:red;">' + name + '</span> ] ？', { icon: 3, title: '系统提示' }, function (index) {
                                                $.post("{{url('admin/power/role/delete')}}",{id:id,'_token':"{{csrf_token()}}"},function(msg){
            ...
            function addTab(id){
                layer.open({
                    ...
                    maxmin: true,
                    type: 2,
                    content: "{{url('admin/power/role/edit/')}}"+'/'+id,
                    area: ['90%', '90%']
                });
            }
            ...
            $('#getAll').click(function(){
                layer.open({
                    ...
                    maxmin: true,
                    type: 2,
                    content: "{{url('admin/power/role/add/')}}",
                    area: ['90%', '90%']
                });
            })
        });
