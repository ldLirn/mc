			...
			<form class="layui-form" action="">
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" name="name" lay-verify="username"  value="{{$data['name']}}"  autocomplete="off" class="layui-input">
					</div>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" name="description" lay-verify="required"  value="{{$data['description']}}"  autocomplete="off" class="layui-input">
					</div>
				</div>
									...
									@foreach($power[$k] as $m=>$n)
											<div class="layui-input-block" style="margin-left:0;overflow: hidden">
												<div style="width: 20%;float: left;" class="item{{$num}}" id="itemMain{{$n->id}}">
													<input type="checkbox"  name="permission1[]" value="{{$n->id}}" @if(in_array($n->id,$data['permission']))checked="checked"@endif  class="value" title="{{$n->description}}">
												</div>
												@foreach($n['child'] as $p)
													@foreach($p as $vo)
													<div style="width: 20%;float: left;" class="item{{$num}} items" id="item{{$vo['id']}}">
														<input type="checkbox"  name="permission1[]" value="{{$vo['id']}}" @if(in_array($vo['id'],$data['permission']))checked="checked"@endif class="value" title="{{$vo['description']}}">
													</div>
														<script>
		...
		<script>
			layui.use(['form', 'layedit', 'laydate'], function() {
				...
				form.on('submit(demo1)', function(data) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{url('admin/power/role/update')}}",
                        data: {id:data.field.id,name:data.field.name,description:data.field.description,level:data.field.level,permission:chkIds,'_token':"{{csrf_token()}}"},
                        dataType: 'json',
